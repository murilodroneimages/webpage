const ConnectionController = {
  ...
  // Ouve o que o Backend responde
  bindSocketEvents: function() {
    this.socket.on('connect', () => {
      this.socket.emit('request_current_status', this.tenantId);
    });

    // O Backend avisa que está gerando e devemos esperar
    this.socket.on('whatsapp_status', (data) => {
      if (data.state === 'DISCONNECTED') this.switchState('disconnected');
      if (data.state === 'STARTING') {
        this.switchState('loading');
        document.getElementById('loading-text').textContent = 'Gerando criptografia do WhatsApp...';
      }
      if (data.state === 'QR_READY') this.switchState('qr-ready');
      if (data.state === 'CONNECTED') this.showConnected(data.number);
    });

    // O Backend enviou a imagem base64
